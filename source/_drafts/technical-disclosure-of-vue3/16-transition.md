...
```javascript
export const Transition = (props, { slots }) => h(
  BaseTransition, 
...
根据我们前面了解到的， `Vue` 会在特定阶段为节点增加或删除特定 `class` 。而这个 `props` 正是为了所谓的 特定的阶段 量身打造的 **钩子** 函数。举个例子，我们需要实现进入节点的 `v-enter-from、v-enter-active、v-enter-to` 类名的添加，我们只需要在 `onEnter` 进入钩子内实现逻辑：
